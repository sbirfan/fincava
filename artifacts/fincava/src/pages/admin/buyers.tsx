...
function OverviewPane({ buyer }: { buyer: BuyerDetail }) {
  ...
  return (
    <div className="space-y-5 text-sm" data-testid="pane-overview">
      ...
      {/* Legacy A–F sections — only shown if buyer used the old profile editor */}
      {ALL_SECTIONS.some((s) => done.has(s)) ? (
        <>
          {/* Section A */}
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.A}</p>
              {done.has("A") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Traceability level" value={buyer.traceabilityLevel ?? "—"} />
              <Field label="Existing Colombia relationship" value={fmtBool(buyer.existingColombiaRel)} />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.B}</p>
              {done.has("B") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Incoterm" value={buyer.preferredIncoterm ?? "—"} />
              <Field label="Volume (MT)" value={buyer.intendedVolumeMt != null ? String(buyer.intendedVolumeMt) : "—"} />
              <Field label="Frequency" value={buyer.importFrequency ?? "—"} />
              <Field label="Trade finance open" value={fmtBool(buyer.tradeFinanceOpen)} />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.C}</p>
              {done.has("C") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.D}</p>
              {done.has("D") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Destination port" value={buyer.destinationPort ?? "—"} />
              <Field label="Logistics partner" value={buyer.logisticsPartner ?? "—"} />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.E}</p>
              {done.has("E") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Previous sourcing channel" value={buyer.prevSourcingChannel ?? "—"} />
              <Field label="Discovery budget band" value={buyer.discoveryBudgetBand ?? "—"} />
              <Field label="Supplier dev open" value={fmtBool(buyer.supplierDevOpen)} />
              <Field label="Early-stage supplier open" value={fmtBool(buyer.earlyStageSupplierOpen)} />
            </div>
            <Field label="Supplier type preference" value={fmtArr(buyer.supplierTypePref)} />
            <Field label="Social impact requirements" value={fmtArr(buyer.socialImpactReqs)} />
          </div>

          {/* Section F */}
          <div className="rounded-lg border border-white/10 bg-white/5 p-3 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-white/40 uppercase tracking-wider">{SECTION_LABELS.F}</p>
              {done.has("F") ? (
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">done</span>
              ) : (
                <span className="text-[10px] text-white/40">pending</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Languages" value={fmtArr(buyer.languagePreference)} />
            </div>
        ...
        <div className="rounded-lg border border-white/10 bg-white/5 p-3">
          <p className="text-xs text-white/40 uppercase tracking-wider mb-1">Legacy profile sections (A–F)</p>
          <p className="text-xs text-white/50">
            Not applicable — this buyer used the new onboarding wizard. Their profile data is in the{" "}
            <span className="text-emerald-400">P2 Onboarding Profile</span> section below.
          </p>
        </div>
      )}
