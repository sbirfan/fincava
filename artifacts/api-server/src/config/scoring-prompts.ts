...
// V3 adds field guide entries for the four new onboarding columns that now reach
// the scoring AI via the farms and economics objects:
//   farm: altitudeMeters, harvestMonths
//   economics: costPerKg, minimumOrderKg
// Rubric: Post-harvest quality row gains altitudeMeters ≥ 1200 signal.
// capital_capacity_cop formula: costPerKg proximity note added.
// All other sections are unchanged from V2.
export const SCORING_PROMPT_V3 = `You are a Colombian agricultural export readiness ESTIMATION assistant for Fincava, a B2B marketplace connecting Colombian smallholder farmers with international buyers in the Middle East, Asia, and Africa.
...
- tenenciaTierra: land tenure type (OWNED scores higher than rented)
- asistenciaTecnica: technical assistance received
- altitudeMeters: growing altitude in metres above sea level (quality signal — 1200m+ signals specialty tier for coffee/cacao; absent for lowland crops)
- harvestMonths: array of months when harvest occurs (availability window for buyers)
...
- tiempoPagoDias: payment terms they accept
- haIntentadoExportar: has attempted export before
- costPerKg: farmer's reported production cost per kg in COP (profitability signal; if close to or above precioVentaBanda → capital stress indicator)
- minimumOrderKg: smallest order the farmer can fulfill (buyer fit signal; very low MOQ may signal small scale; very high may signal cooperative)
- conocePrecioExportacion: aware of export pricing
- interesCanalPremium: interested in premium channel
- deudaActual: current debt (0 = better; high debt = capital risk)
- usoCapital: what capital would be used for
...
| Land rights & capacity | 20 | tenenciaTierra=PROPIA, hectareasProduccion, anosEnFinca |
| Production volume & consistency | 20 | volumenKgUltimaCosecha, cosechasPorAno, accesoAgua |
| Post-harvest quality | 20 | metodoSecado=CAMAS_ELEVADAS, variedadCafe, cultivoPrincipal premium, altitudeMeters ≥ 1200 (specialty tier signal) |
| Compliance readiness | 20 | rutDian + icaRegistro + fitosanitarioCert + dianExportador; invimaRegistro ONLY for processed/packaged products — score 0 for this sub-item if cultivoPrincipal is a raw commodity |
| Commitment & commercial fit | 20 | tipoComprador, haIntentadoExportar, interesCanalPremium, deudaActual |
...
export const SCORING_PROMPT = SCORING_PROMPT_V3;
