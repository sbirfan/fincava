# CC-1D — DIAN RUT Supplier Flow (Corrected v2)
...
Add `supplierId` to the Express `Request` type augmentation (look for an existing `types/express.d.ts` or similar and add `supplierId?: number`).
...
import { db } from '../../../lib/db/src'; // adjust to actual db import
import {
...
import { logger } from '../lib/logger'; // use actual import
import { sendAdminSupportAlert } from '../lib/email'; // use actual or create
...
Open `artifacts/api-server/src/routes/index.ts` (or wherever routes are mounted). Add:

```typescript
import supplierComplianceRouter from './supplierCompliance';

// Mount under /api/supplier/compliance
app.use('/api/supplier/compliance', supplierComplianceRouter);
// or if routes are mounted differently, follow the existing convention for supplier routes
```
