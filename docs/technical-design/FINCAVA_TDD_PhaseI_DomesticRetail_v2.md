...
New retail tables use **integer centavos (COP)** for all monetary columns, with an explicit `currency` text column (default `'COP'`) alongside every monetary amount. Existing B2B tables (`orders.totalUSD`, `order_items.pricePerKg`, `order_items.totalUSD`, `artifacts/api-server/src/services/fee-service.ts` return values) remain as PostgreSQL `real` and are **not changed** by Phase I migrations.
...
### 0.5 Production State Awareness

As of this document, the platform has zero orders in production. The buyer intent flow (`POST /api/orders/buyer/intent`) has never executed against live data. Retail orders will share the `orders` table with B2B inquiries and will co-discover production behavior in shared code paths (fee computation, order status queries, admin order views).

Before Day 0 of retail launch, run the existing B2B test suite plus a B2B end-to-end smoke test on staging to validate the shared `orders` table path before any retail traffic hits it. This smoke test is also specified in Section 10.5 (Day -7 migration sequence).
...
Full file contents to create at `artifacts/api-server/src/lib/interaction-types.ts`:

```typescript
// artifacts/api-server/src/lib/interaction-types.ts
export const INTERACTION_TYPES = {
  FORM_SUBMISSION: "FORM_SUBMISSION",  // existing value, do not change
...
Note: `WHATSAPP_SENT` was in a prior draft but is not included — WhatsApp send events are logged at the Twilio level; writing to `interactions` for every outbound message adds table writes without corresponding query value. If interaction-based WhatsApp audit is needed, add `WHATSAPP_SENT` then.
...
| `interactions` table | Write (field-officer visits) | Free-text `interactionType = 'FARM_VISIT'` per `artifacts/api-server/src/lib/interaction-types.ts` |
...
| `artifacts/api-server/src/services/fee-service.ts` | No change | B2B fee only; retail has its own commission logic |
...
### 1.5 B2B / Retail Separation Map

The purpose of this section is to make B2B/retail separation a verifiable constraint. Any future feature proposing to modify a shared table must justify the change against this map.

#### Shared Tables (additive columns only — never destructive changes)

| Table | B2B use | Retail addition | Separation mechanism |
|---|---|---|---|
| `orders` | B2B inquiry → delivery flow | Retail orders use same table | `retail_order_details` child table carries all retail-specific fields; `channel` column (`'b2b'` \| `'retail'`) distinguishes rows; retail enum values appended |
| `products` | B2B catalog | Retail SKU attributes | `retail_enabled = false` by default; retail columns nullable and ignored by B2B queries |
| `origin_stories` | B2B marketplace content | Buyer-facing bilingual copy | `buyer_copy_es/en`, `farmer_voice_es/en` columns added; existing B2B story fields untouched |

#### Retail-Only Tables (prefixed `retail_*`, no B2B dependencies)

- `retail_buyer_profiles` — retail buyer profile (address, language, filter prefs)
- `retail_order_details` — retail-specific order fields (shipping snapshot, tracking, farmer payment)
- `retail_harvest_updates` — farmer harvest update posts for retail waitlist and product page
- `retail_shipping_zones` — domestic shipping rate table (department × weight class)
- `retail_payment_transactions` — gateway-agnostic payment ledger (Wompi V1, Stripe Phase II)
- `retail_waitlists` — waitlist signups per product (guest + authenticated)
- `retail_auth_tokens` — magic-link and SMS OTP tokens

#### Reused Infrastructure (shared, not duplicated)

| System | Shared component | Retail access pattern |
|---|---|---|
| Compliance engine | CC-1 family tables | Read-only; filter `sellable_status = PUBLISHED` |
| AI pipeline | `lib/anthropic.ts` — 6 model constants | Reused as-is; no parallel AI infrastructure |
| WhatsApp | `lib/whatsapp.ts` | Farmer notifications only |
| Email | Resend via `lib/email.ts` | Buyer + admin notifications |
| Auth | Session/cookie pattern | Extended with magic-link; no parallel auth system |

#### New Retail-Specific Infrastructure (additive — not replacing anything)

- `artifacts/api-server/src/lib/sms.ts` — Twilio SMS wrapper (OTP fallback; distinct from WhatsApp)
- `artifacts/api-server/src/lib/cron.ts` — node-cron scheduled tasks (SLA sweep, account deletion)
- `artifacts/api-server/src/lib/interaction-types.ts` — canonical interactionType constants
- `/api/retail/*` routes — all retail backend endpoints
- `/tienda/*` frontend routes — all retail buyer-facing pages

#### Spin-Off Readiness Note

Physical separation (separate database, separate service) is not implemented in Phase I. Readiness is achieved through clean module ownership so that a future spin-off is a deployment change, not a schema change.

If retail spins off, the migration path is:
1. Extract `retail_*` tables to a new database.
2. Fork `orders` and `products` with a data migration for retail rows.
3. Replace cross-table queries with API calls.

This is a known cost accepted at V1 in exchange for development simplicity.

---

## 2. Database Architecture

### 2.1 interaction-types.ts — Canonical Constants
...
#### 2.2.2 `origin_stories` — Farmer Approval Gate (Section 3.18) + Bilingual Columns
...
Bilingual column additions:

```typescript
// Additions to originStoriesTable
farmerVoiceEs: text("farmer_voice_es"),        // farmer's own words in Spanish
farmerVoiceEn: text("farmer_voice_en"),        // English translation; may lag ES at launch; product page falls back to ES with disclosure if null
buyerCopyEs: text("buyer_copy_es"),            // buyer-facing Spanish copy (tasting notes, brewing recommendations)
buyerCopyEn: text("buyer_copy_en"),            // buyer-facing English copy; may lag ES at launch
translatedBy: text("translated_by"),           // 'human' | 'sonnet_assisted' | null
```

Migration SQL to append to existing origin_stories migration:
```sql
ALTER TABLE origin_stories
  ADD COLUMN farmer_voice_es text,
  ADD COLUMN farmer_voice_en text,
  ADD COLUMN buyer_copy_es text,
  ADD COLUMN buyer_copy_en text,
  ADD COLUMN translated_by text;
```

Rollback SQL:
```sql
ALTER TABLE origin_stories
  DROP COLUMN IF EXISTS farmer_voice_es,
  DROP COLUMN IF EXISTS farmer_voice_en,
  DROP COLUMN IF EXISTS buyer_copy_es,
  DROP COLUMN IF EXISTS buyer_copy_en,
...
export const retailBuyerProfilesTable = pgTable("retail_buyer_profiles", {
  ...
  languagePref: text("language_pref").notNull().default("es"),  // 'es' | 'en'
...
Migration SQL to add `language_pref` column (append to existing retail_buyer_profiles migration):
```sql
ALTER TABLE retail_buyer_profiles
  ADD COLUMN language_pref text NOT NULL DEFAULT 'es';
```
...
Migration SQL:
```sql
CREATE TABLE retail_harvest_updates (
  id                serial PRIMARY KEY,
  product_id        integer NOT NULL REFERENCES products(id) ON DELETE CASCADE,
...
CREATE INDEX idx_retail_harvest_updates_product_id ON retail_harvest_updates (product_id);
```

Rollback SQL:
```sql
DROP TABLE IF EXISTS retail_harvest_updates;
```
...
0026_retail_harvest_updates.sql             — CREATE TABLE retail_harvest_updates
```

Migrations 0016 and 0017 are safe column additions (all new columns are nullable or have defaults). Migration 0018 is forward-only. Migrations 0019–0024 and 0026 create independent tables and can be rolled back individually by dropping the table. The drop order is the reverse of creation.
...
- **Behavior:**
  ...
  4. For first-time guest verification (token's email matches no existing `users` row):
     1. Create a `users` row with `role = 'BUYER'`, `email = tokenEmail`, `emailVerified = true`.
     2. Create a `retail_buyer_profiles` row linked to the new userId.
     3. Populate `retail_buyer_profiles.firstName` from `retail_order_details.shippingName` for this email if a prior guest order exists (COALESCE from the most recent matching row; null if none).
     This makes magic-link verification a unified login-or-register flow — no separate registration screen is required.
  5. Create session (set-cookie, same mechanism as existing auth).
...
- **Event handling:**
  ...
  - `transaction.updated` with `status = DECLINED` / `ERROR`: update to FAILED. Before restoring stock: check `orders.status`. Restore `products.retailStockUnits` ONLY if `orders.status` is still `INQUIRY` or `AUTHORIZED`. If the order has already moved to a terminal state (`VOIDED`, `REFUNDED`, `CANCELLED`), skip stock restoration — the terminal-state transition path has already performed it. This prevents a double-restoration race condition between the Wompi webhook handler and the daily SLA sweep cron job.
...
import { db, retailPaymentTransactionsTable, ordersTable, retailOrderDetailsTable, productsTable } from "@workspace/db";
  ...
  cron.schedule("0 7 * * *", async () => {
    ...
    for (const txn of overdueAuthorizations) {
      try {
        ...
        // Restore stock for each voided order
        const voidedOrders = [{ id: txn.orderId }];
        for (const order of voidedOrders) {
          const [details] = await db
            .select({ productId: retailOrderDetailsTable.productId, unitQuantity: retailOrderDetailsTable.unitQuantity })
            .from(retailOrderDetailsTable)
            .where(eq(retailOrderDetailsTable.orderId, order.id));
          if (details) {
            await db
              .update(productsTable)
              .set({ retailStockUnits: sql`retail_stock_units + ${details.unitQuantity}` })
              .where(eq(productsTable.id, details.productId));
          }
        }
...
- Saves post to `retail_harvest_updates`. See Section 2 for full table spec.
...
1. Void all AUTHORIZED `retail_payment_transactions` for orders tied to this product (calls WompiAdapter.void(); restores stock units). Scope: void/refund only orders whose `retail_payment_transactions.createdAt` is after the previous successful harvest's stock replenishment event (`products.lastReplenishedAt`). Orders from prior completed harvests are not affected. Engineering must use this scoping condition in the query to avoid touching settled orders from earlier harvest cycles.
2. Issue refunds for any CAPTURED transactions (same scope).
...
All buyer-facing transactional templates exist in both ES (Spanish) and EN (English) variants. Template selection is driven by `retail_buyer_profiles.language_pref` for the receiving buyer. Farmer-facing WhatsApp templates and admin console notifications remain Spanish-only.

**T-B1: Order Confirmation (email) (ES + EN)**
- Language variants: T-B1-es, T-B1-en
...
**T-B2: Waitlist Confirmation (email or WhatsApp) (ES + EN)**
- Language variants: T-B2-es, T-B2-en
...
**T-B3: Waitlist Mid-Cycle Update (email or WhatsApp) (ES + EN)**
- Language variants: T-B3-es, T-B3-en
...
**T-B4: Waitlist At-Ready Conversion (email or WhatsApp) (ES + EN)**
- Language variants: T-B4-es, T-B4-en
...
**T-B5: Harvest Failure Dual-Option Exit (email) (ES + EN)**
- Language variants: T-B5-es, T-B5-en
...
**T-B6: Shipping Notification (email) (ES + EN)**
- Language variants: T-B6-es, T-B6-en
...
**T-B7: Shipping Delay Notification (email) (ES + EN)**
- Language variants: T-B7-es, T-B7-en
...
**T-B8: Post-Delivery Review Request (email) (ES + EN)**
- Language variants: T-B8-es, T-B8-en
...
**T-B9: Magic Link (email) (ES + EN)**
- Language variants: T-B9-es, T-B9-en
...
**T-B10: SMS OTP (SMS) (ES + EN)**
- Language variants: T-B10-es, T-B10-en
...
Day -7 (one week before launch):
  ...
  Verify: no B2B queries broken (run existing test suite). Also run a B2B end-to-end smoke test on staging: create a test supplier, create a test product, call POST /api/orders/buyer/intent, confirm an orders row is created with status = 'INQUIRY' and incoterm = 'FOB'. This validates that the shared orders table path works correctly before any retail orders land on it.
...
Day -3:
  ...
  Run 0026 (retail_harvest_updates)
...
**Origin story language extensibility:** `origin_stories` now carries `farmer_voice_es/en`, `buyer_copy_es/en`, and `translated_by` columns added in Phase I. Phase II may add a child `origin_story_translations` table for additional languages without touching Phase I columns.

**`artifacts/api-server/src/lib/interaction-types.ts`:** extending with new interaction types for Phase II is a code change only (no migration).
...
- AI-assisted translation of authored content for Phase II international surfaces
...
**v1 (May 2026):** Initial TDD produced covering all 12 technical sections for Colombian domestic retail launch.

**v2 (May 2026):** Sixteen corrections applied.

Four drift corrections from codebase verification: (A) buyer intent route path corrected to `POST /api/orders/buyer/intent`; (B) fee-service.ts module path corrected to `artifacts/api-server/src/services/fee-service.ts`; (C) feeStatus values confirmed as `WAIVED | PENDING | INVOICED | PAID`; (D) `interaction-types.ts` path made consistent at `artifacts/api-server/src/lib/interaction-types.ts`.

Eight refinements: Wompi DECLINED handler state-check added to prevent double-restoration race condition with SLA sweep cron; harvest failure scope clarified to target only orders from the failed harvest cycle (`retail_payment_transactions.createdAt` after `products.lastReplenishedAt`); `retail_harvest_updates` specified as a proper Phase I table with full schema, migration, and rollback SQL; magic-link verification extended with first-time user-and-profile creation branch; Section 0.5 Production State Awareness added (zero live transactions at time of writing; B2B smoke test required before Day 0); SLA sweep stock restoration implemented with explicit Drizzle code replacing placeholder comment; Day -7 migration sequence expanded with B2B end-to-end smoke test spec; `interaction-types.ts` constants file fully specified including `CLAIM_INITIATED` and rationale for excluding `WHATSAPP_SENT`.

Three bilingual additions: `origin_stories` columns `farmer_voice_es/en`, `buyer_copy_es/en`, `translated_by` added with full Drizzle schema, migration, and rollback SQL; `retail_buyer_profiles.language_pref` column added; buyer notification templates explicitly specified as ES + EN variants per `language_pref`; Phase II handoff list corrected (English UI surfaces removed — now V1).

One architectural separation map added (Section 1.5): explicitly lists shared tables, retail-only tables, reused infrastructure, new retail-specific infrastructure, and spin-off readiness note.

Companion document `FINCAVA_DesignThinking_Phase2_v2.1.md` produced in the same session with four matching corrections.
