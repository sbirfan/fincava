...
1. There is no `payment_milestones` table. Payment state today is `orders.feeStatus` (PENDING | WAIVED | COLLECTED | EXEMPT). Wompi is greenfield — Phase 2 ideation is free to introduce a payment lifecycle without working around legacy infrastructure.
...
Today, `POST /api/buyers/intent` creates an `orders` row with `status = INQUIRY` and a `supplierId` FK, plus an admin alert email via Resend. This is the only buyer-to-supplier commitment flow that exists in code. It is B2B-shaped (intended for importer inquiries) but has not yet processed a live transaction.
...
**Builds on:** `orders` table, `order_status` enum, `lib/fee-service.ts` per-order fee computation (4% standard, first 10 orders WAIVED), the existing `POST /api/buyers/intent` precedent.
...
**Builds on:** `orders.feeStatus` column convention (PENDING | WAIVED | COLLECTED | EXEMPT will need to accept new values such as AUTHORIZED, CAPTURED, REFUNDED — to be designed in Phase 3). New conceptual ledger `payment_transactions` referenced earlier as part of the order architecture.
...
**4.5 Speak in the language of the participant.** Farmers see Spanish. Buyers see Spanish (V1). The Buyer Onboarding P2 (B2B) flow remains untouched (its own bilingual concerns apply to importers). The runtime `messages.translatedContent` capability is exposed in the buyer-farmer thread UI for farmer↔buyer conversations.
...
**Unknown: Buyer-side language preference enforcement.** Phase 1 V1 is Spanish-only. If an international buyer accidentally arrives at the Colombian retail marketplace, the experience is partially in Spanish. Phase 2 (Phase 1 of this document; the design series) addresses this only via the language preference column, not via locale routing.
...
**Content.** Origin stories and product copy translate to English at international launch. The translation is authored by a human translator, not by `TRANSLATION_MODEL`. The `origin_stories` table should accommodate per-language content (e.g., `storyEs`, `storyEn`) or a child translation table when the time comes.
